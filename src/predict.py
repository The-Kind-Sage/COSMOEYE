...
def resolve_image_path(sample_file_name):
    ...
    candidates = [
        os.path.join(PATHS.TEST_IMG_DIR,   sample_file_name),
        os.path.join(PATHS.TRAIN_IMG_DIR,  sample_file_name),
    ]
...
def resolve_mask_path(sample_file_name):
    ...
    for path in (os.path.join(PATHS.TEST_MASK_DIR,  sample_file_name),
                 os.path.join(PATHS.TRAIN_MASK_DIR, sample_file_name)):
        if os.path.exists(path):
            return path
    return None
...
def save_overlay(sample_file_name, raw_post_rgb, binary_mask, probability_mask,
    ...
    if out_dir is None:
        out_dir = PATHS.PREDICTION_OVERLAYS_DIR
    try:
        import matplotlib
        matplotlib.use("Agg")
...
def save_insight_panels(sample_file_name, raw_post_rgb, binary_mask,
                        probability_mask, out_dir=None):
    """Render the insight panels as separate PNG files and return their paths."""
    if out_dir is None:
        out_dir = PATHS.PREDICTION_OVERLAYS_DIR
    try:
        import matplotlib
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "batch":
        ...
        mock_path = os.path.join(PATHS.TEST_IMG_DIR, mock_test_file)
