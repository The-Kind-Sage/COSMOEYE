...
NDVI_CHANNEL = 13
# Import from paths.py so the location is defined in exactly one place.
# Fall back to the legacy location if paths.py isn't available (e.g. when
# dataset.py is imported in isolation during a quick sanity check).
try:
    from paths import PATHS as _PATHS
    NORM_STATS_PATH = _PATHS.NORM_STATS
except ImportError:
...
class LandslideDataset(Dataset):
    def __init__(self, img_dir=None,
        ...
        try:
            ...
            _default_img  = _P.TRAIN_IMG_DIR
            _default_mask = _P.TRAIN_MASK_DIR
        except ImportError:
            _default_img  = "./data/TrainData/img/"
            _default_mask = "./data/TrainData/mask/"
        self.img_dir  = img_dir  if img_dir  is not None else _default_img
        self.mask_dir = mask_dir if mask_dir is not None else _default_mask
